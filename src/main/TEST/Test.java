...
public class Test {
    ...
    //term frequency in a file, frequency of each word//只是转化吗
    public static HashMap<String, Float> tf(ArrayList<String> cutwords){
        HashMap<String, Float> resTF = new HashMap<String, Float>();

        int wordLen = cutwords.size();
        HashMap<String, Integer> intTF = Test.normalTF(cutwords);//统计出现次数
...
//          System.out.println(entry.getKey().toString() + " = "+  Float.parseFloat(entry.getValue().toString()) / wordLen);
    ...
    public static HashMap<String,HashMap<String, Float>> tfAllFiles(String dirc) throws IOException{
        ...
        for(String file : filelist){//遍历每个txt
    ...
    public static HashMap<String, Float> idf(HashMap<String,HashMap<String, Float>> all_tf) throws IOException{//all_tf为词频
        HashMap<String, Float> resIdf = new HashMap<String, Float>();
        HashMap<String, Integer> dict = new HashMap<String, Integer>();
        int docNum = FileList.size();

        for(int i = 0; i < docNum; i++){//在所有txt中进行统计
...
//          System.out.println(entry.getKey().toString() + " == " + value);
    ...
    public static void main(String[] args) throws IOException {
        ...
        HashMap<String,HashMap<String, Float>> all_tf = tfAllFiles(inputpath);//返回tf
        System.out.println();
